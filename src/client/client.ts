...
export const updateJiraIssue = async (
	...
	await new Promise<void>((resolve, reject) => {
		const url = new URL(
			`${jiraApiUrl}/rest/api/3/issue/${encodeURIComponent(issueIdOrKey)}`,
		);

		// Build update-safe payload: omit project and issuetype which cannot be changed via update
		const updateFields: Record<string, unknown> = {};
		if (payload.fields.summary !== undefined)
			updateFields.summary = payload.fields.summary;
		if (payload.fields.description !== undefined)
			updateFields.description = payload.fields.description;
		if (payload.fields.labels !== undefined)
			updateFields.labels = payload.fields.labels;
		if (payload.fields.components !== undefined)
			updateFields.components = payload.fields.components;
		if (payload.fields.assignee !== undefined)
			updateFields.assignee = payload.fields.assignee;
		if (payload.fields.priority !== undefined)
			updateFields.priority = payload.fields.priority;
		if (payload.fields.fixVersions !== undefined)
			updateFields.fixVersions = payload.fields.fixVersions;
